...
$sentencia = $conn->prepare("SELECT * FROM grupo WHERE idGrupo = ?;");
$sentencia->execute([$id]);
$grupo = $sentencia->fetch(PDO::FETCH_OBJ);
if($grupo == FALSE){
    echo "No existe ningun grupo con ese id";
    exit();
}
	...
	<div class="card grey lighten-2">
		<div class="row">
				...
				<table>
					...
					<tbody>
						...
						while ($valores = mysqli_fetch_array($query)) {
							...
							echo '<td>
                            <form method="post" action="groupUp.php">
                            <input type="hidden" class="form-control" name="idTeacher" value="<?php echo $grupo->idGrupo?>">
							...
							echo '<td>
							<form method="post" action="GroupDelete.php">
			...
			<div class="card-action right">
				<a href="RGroups.php" class="left waves-effect waves-light-green btn green">Download report<i class="material-icons right">file_copy</i></a>
			</div>
		</div>
	</div>
</div>
